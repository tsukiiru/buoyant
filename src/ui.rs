  ...
  pub fn ui(&mut self, main_ui: &mut eframe::egui::Ui) {
    ...
    CentralPanel::default().show(main_ui, |ui| {
      for (ri, row) in self.panels_manager.panels.iter().enumerate() {
        StripBuilder::new(ui)
          .size(Size::relative(self.panels_manager.height_proportion[ri]))
                ...
                builder
                  ...
                  .vertical(|mut strip| {
                    strip.cell(|ui| {
                      // address bar
                      let mut button = ui
                        .add(Button::new(RichText::new("<").size(14.0)).fill(Color32::TRANSPARENT));
      ...
      && kind == OverlayKind::Paste
    {
      ...
      modal_widget.show(&ctx, |ui| {
        ...
        ui.vertical(|ui| {
          if ui
            .add(Button::new("replace").shortcut_text(ctx.format_shortcut(&keybinds.choice_0)))
            .clicked()
          {
        ...
        ui.vertical(|ui| {
          if ui
            .add(Button::new("duplicate").shortcut_text(ctx.format_shortcut(&keybinds.choice_1)))
            .clicked()
          {
      ...
      && kind == OverlayKind::Delete
    {
      ...
      modal_widget.show(&ctx, |w| {
        ...
        w.horizontal(|u| {
          if u
            .add(Button::new("yeah").shortcut_text(ctx.format_shortcut(&keybinds.choice_0)))
            .clicked()
          {
            req_overlay_choice = Some(0);
          }
          if u
            .add(Button::new("no").shortcut_text(ctx.format_shortcut(&keybinds.choice_1)))
            .clicked()
